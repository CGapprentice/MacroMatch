...
def create_routine():
    try:
        ...
        routine = Routine(
            ...
            speed=int(data.get('speed')) if data.get('speed') is not None else None,
            speedUnit=data['speedUnit'],
            distance=int(data.get('distance')) if data.get('distance') is not None else None,
            distanceUnit=data['distanceUnit'],
            highIntensity=int(data.get('highIntensity')) if data.get('highIntensity') is not None else None,
            highIntensityUnit=data['highIntensityUnit'],
            lowIntensity=int(data.get('lowIntensity')) if data.get('lowIntensity') is not None else None,
            lowIntensityUnit=data['lowIntensityUnit'],
            restTime=int(data.get('restTime', None)) if data.get('restTime') is not None else None,
